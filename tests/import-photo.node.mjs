...
for (const withMetadata of [true, false]) {
  test(`import ${withMetadata ? "with photographic and location metadata" : "without EXIF"}`, async () => {
    ...
    try {
      ...
      writeFileSync(join(root, "mocks/codex"), '#!/bin/sh\nprintf "%s\\n" \'A "pigeon" with \\ feathers.\'\n', { mode: 0o755 });
